...
public abstract class GenericConstant {
	...
	public String getKey(String value) {
		return getMapKeyValue().entrySet().stream().filter(entry -> Objects.equals(entry.getValue(), value)).findFirst()
				.get().getKey();
	}
	...
	public List<String> getKeys() {
		return getMapKeyValue().entrySet().stream().map(entry -> entry.getKey()).collect(Collectors.toList());
	}
